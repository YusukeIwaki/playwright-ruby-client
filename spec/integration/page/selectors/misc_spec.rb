...
RSpec.describe 'selector/misc' do
  it 'should work for open shadow roots', sinatra: true do
    with_page do |page|
      page.goto("#{server_prefix}/deep-shadow.html")
      expect(page.eval_on_selector('id=target', 'e => e.textContent')).to eq('Hello from root2')
      expect(page.eval_on_selector('data-testid=foo', 'e => e.textContent')).to eq('Hello from root1')
      expect(page.eval_on_selector_all('data-testid=foo', 'els => els.length')).to eq(3)
      expect(page.query_selector('id:light=target')).to be_nil
      expect(page.query_selector('data-testid:light=foo')).to be_nil
      expect(page.query_selector_all('data-testid:light=foo')).to be_empty
    end
  end

  it 'should click on links in shadow dom', sinatra: true do
    with_page do |page|
      page.goto("#{server_prefix}/shadow-dom-link.html")
      expect { page.click('#inner-link') }.to change {
  ...
  it 'should escape the scope with >>' do
    with_page do |page|
      page.content = "<div><label>Test</label><input id='myinput'></div>"
      expect(page.eval_on_selector('label >> xpath=.. >> input', 'e => e.id')).to eq('myinput')
    end
  end
  ...
  it 'should work with internal:has-not=' do
    with_page do |page|
      page.content = "<section><span></span><div></div></section><section><br></section>"
      expect(page.eval_on_selector_all('section >> internal:has-not="span"', 'els => els.length')).to eq(1)
      expect(page.eval_on_selector_all('section >> internal:has-not="span, div, br"', 'els => els.length')).to eq(0)
      expect(page.eval_on_selector_all('section >> internal:has-not="br"', 'els => els.length')).to eq(1)
      expect(page.eval_on_selector_all('section >> internal:has-not="span, div"', 'els => els.length')).to eq(1)
      expect(page.eval_on_selector_all('section >> internal:has-not="article"', 'els => els.length')).to eq(2)
    end
  end
